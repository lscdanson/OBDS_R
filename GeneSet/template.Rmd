...
all_genes_df <- read_csv("/t1-data/project/obds/dloi/2_base_r/projects/DEseq2/CD8_vs_CD4_DE_results_lfcshrinkage_allgenes.csv")
...
all_gene_ids <- all_genes_tested_df$ensemblID
...
gost_results_obj <- gost(list(all_gene_ids, upreg_gene_ids), organism = "mmusculus", multi_query = T)
...
gostplot(gost_results_obj, capped = T, interactive = F)
```

```{r}
# what does as_short_link = FALSE do in gost() function? 
gost(list(all_gene_ids, upreg_gene_ids), organism = "mmusculus", multi_query = T, as_short_link = F)
```
