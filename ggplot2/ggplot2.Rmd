...
> Answer: a grey box. Coz you haven't specified anything
...
```{r}
ggplot(diamonds, aes(x = carat, y = price)) +
         geom_point(aes(colour=cut)) + geom_smooth() 
...
```{r}
msleep_plot <- ggplot(msleep, aes(x=order)) +
  geom_bar()
...
msleep_plot + theme(axis.text.x= element_text(size=10, hjust=1, angle=90))
...
library(cowplot)
first_row <- cowplot::plot_grid(obds_diamonds, obds_chickweight, labels=c("A","B"), nrow=1, ncol=2)
  
first_row
second_row <- cowplot::plot_grid(obds_msleep, labels="C", nrow=1)
  
super_plot <- cowplot::plot_grid(first_row,second_row, nrow=2,ncol=1)
  
super_plot
ggsave("cowplot.png",plot=super_plot, height=12, width =15, units="in")
```
