...
#alternative (doesnt seem to work?)
...
logcounts_matrix <- as.data.frame(logcounts_matrix) #coz tibble doesnt handle rownames, we need to convert it to data frame first

logcounts_matrix <- column_to_rownames(logcounts_matrix, var="gene")
logcounts_matrix <- as.matrix(logcounts_matrix) #matrix is gonna make it easy for the apply() function later on
...
row1_test
str(row1_test) #base R objects are basically a list of classes
row1_test$p.value #so you can extract the p-value by assigning a dollar sign
```
