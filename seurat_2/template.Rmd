...
```{r}
seurat_after_qc <- SCTransform(seurat_after_qc, variable.features.n = 2000, vars.to.regress = "percent_mt")
```

Where is the new normalisation stored?
Answer: As an assay called "SCT". SCT trims away some genes --> dimension of the matrix becomes different.
...
seurat_after_qc[[]] #returns metadata
```
...
Answer: Added two columns - new nCounts and new nFeatures
...
```{r}
VlnPlot(seurat_after_qc, features = c("nCount_RNA", "nCount_SCT"), ncol=2, same.y.lims = T, group.by = "orig.ident")
VlnPlot(seurat_after_qc, features = c("nFeature_RNA", "nFeature_SCT"), ncol=2, same.y.lims = T, group.by = "orig.ident")
...
```{r}
ElbowPlot(seurat_after_qc, reduction = "sct.pca", ndims = 50)
```
...
#can see which cluster in RNA matches with which cluster in SCT 
```
...
p3 <- DimPlot(seurat_after_qc, reduction = "umap", pt.size = NULL, group.by = "RNA_snn_res.0.5") + ggtitle("RNA UMAP")
p4 <- DimPlot(seurat_after_qc, reduction = "sct.umap", pt.size = NULL, group.by = "SCT_snn_res.0.5") + ggtitle("SCT UMAP")
p3 + p4
...
FeaturePlot(seurat_after_qc, features = c("LYZ","FCGR3A","CD4","CD8A","GNLY","MS4A1","CST3","PPBP"), reduction = "umap", label = T)
FeaturePlot(seurat_after_qc, features = c("LYZ","FCGR3A","CD4","CD8A","GNLY","MS4A1","CST3","PPBP"), reduction = "sct.umap", label = T)
```
...
```{r}
hto12.umis <- readRDS("/project/obds/ccohen/single-cell/seurat_2/data/hto12_umi_mtx.rds")
dim(hto12.umis) #gene as rowname; cell barcode as colname
head(hto12.umis)
```
...
```{r}
hto12.htos <- readRDS("/project/obds/ccohen/single-cell/seurat_2/data/hto12_hto_mtx.rds")
dim(hto12.htos)
head(hto12.htos) #cell barcode as rowname; HTO as colname
```
...
head(hto12.umis.common) #gene as rowname; cell barcode as colname
```
...
hto12_object <- NormalizeData(hto12_object, assay = "HTO", normalization.method = "CLR" , margin=1) #CLR is designed to normalise the bimodal data like HTO data
```
...
hto12_object <- HTODemux(hto12_object,
                         assay = "HTO") #positive.quantile is the p-value cut off, default is 0.99
```
...
RidgePlot(hto12_object, assay = "HTO", features = c("KG1-A", "THP1-A", "K562-A"))
```
