...
> Answer: tibble, 150x5, dbl (decimal point numbers) and chr (characters)
...
iris_raw %>% 
    group_by(species) %>%
  summarise(across(where(base::is.numeric), mean))
...
iris_raw %>% 
    group_by(species) %>%
  summarise(across(where(base::is.numeric), mean)) %>% 
    filter(sepal_length < 6) %>% as_tibble()
...
iris_raw %>%
  ...
  summarise(across(where(base::is.numeric), mean)) %>% 
    ...
    arrange(desc(sepal_length)) %>% as_tibble()
...
iris_long2 <- iris_raw %>% 
    mutate(
      ID = row.names(iris_raw)
      #Alternative: ID = as.character(seq(1, nrow(iris_row)))
    ) %>% 
    pivot_longer(where(is.numeric), names_to = "variable")
iris_long2
```
...
iris_wide <- iris_long2 %>% 
  pivot_wider(names_from = variable, id_cols = ID)
...
```{r}
library(readxl)
excel_sheets("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx") #tells you the names of the sheets (tabs) in the excel file
exprs_assay <- read_excel("/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", "exprs") #then you specify the sheet
