...
> Answer:dgCMatrix (not a Seurat object initially)
> 
...
```{r}
read10x_data[1:15, 1:6] #Seurat by default uses gene symbol as identifier, but you can change this default to using ensembl ID (in case some genes don't have a gene symbol)
```

> Answer: "." represents zeros. You need to store 3 values - which row, which column and which value. By only including this information for non-zero values it saves memory space as there are so many zeros.
> 
...
> Answer: 33538 features and 5155 cell barcodes (cells)
> 
...
seurat_object <- CreateSeuratObject(
  counts = read10x_data,
  project = "pbmc5k", #this is an arbitrary identifier that you name
  min.cells = 3,
  min.features = 200
)
seurat_object
...
```{r}
dim(read10x_data) - dim(seurat_object) #you can subtract matrices to view the features and cells filtered off
```
...
GetAssayData(seurat_object, slot="data",assay="RNA")[1:6,1:6]
```
...
```{r}
head(FetchData(seurat_object, vars="LYZ", slot="data"))
...
```{r}
VlnPlot(seurat_object, features = c("nCount_RNA", "nFeature_RNA")) #automatically will look for meta.data
```
...
VlnPlot(seurat_object, feature="percent_mt")
```
...
seurat_after_qc <- NormalizeData(seurat_after_qc, 
                                 ...
                                 scale.factor=10000) #scale.factor is like the million in TPM, but 10,000 instead of million coz single-cell data seldom gets a million counts
...
```{r}
ggplot_lyz_raw <- ggplot(FetchData(seurat_after_qc, var = "LYZ", slot = "counts"), aes(x=LYZ)) +
    geom_histogram(   ) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()

ggplot_lyz_normalised <- ggplot(FetchData(seurat_after_qc, var ="LYZ", slot = "data"), aes(x=LYZ)) +
    geom_histogram(   ) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
...
> Answer: We use as internal QC for our own record.
> 
...
) #regressing out library size and mitochondrial gene fraction to account for the differences/variations caused by these two sources which are unwanted
...
saveRDS(seurat_after_qc, file="Seuratobject_after_qc.RDS")
```
...
```{r}
seurat_after_qc <- FindNeighbors(seurat_after_qc, dims=1:20, k.param = 20)
```
...
```{r}
UMAPPlot(seurat_after_qc, group.by="RNA_snn_res.0.5", label=T)
```
...
> Answer: pct.2 is the percentage of cells outside that cluster that are expressing that marker
> The same gene can appear multiple times in different clusters 
...
seurat_markers_all %>% 
  ...
  arrange(desc(avg_log2FC)) %>% 
            slice_head(n=10) %>% pull(gene) -> seurat_markers_top_ten_c3
...
markers_top10_clusters <- seurat_markers_all %>%
    group_by(cluster) %>% filter(p_val_adj < 0.05) %>% arrange(desc(avg_log2FC)) %>% 
